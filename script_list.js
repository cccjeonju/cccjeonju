$(function(){
	...
	$.ajax({
		...
		for (var i = 0; i < sum; i++) { // 전체 강의 목록을 콘솔에 출력. 
		    ...
		    $('#subjectCode').append( $('<option value="' + subject_list[i].c[4].v.toString() + '">[' + subject_list[i].c[2].v.toString() + '] '+ subject_list[i].c[6].v.toString() + ' / ' + subject_list[i].c[5].v.toString() + '</option>\n') );
